    ...
    <div style={{ minHeight: '100vh', padding: '76px 20px 100px',
      ...
      <motion.div style={{ display: 'flex', flexDirection: 'column', gap: 12,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <Btn variant="secondary" full onClick={() => { play('click'); go('leaderboard') }} onHover={() => play('hover')}>
            🏆 Board
          </Btn>
...
function ResultsScreen({ result, go, play, dark }) {
  ...
  // Auto-submit score once on mount
  useEffect(() => {
    if (submitted.current) return
    submitted.current = true
    submitScore({
      name: mode.name, jersey: mode.jersey,
      score, correct, total, maxStreak,
      category: mode.cat,
      ...
      if (res.ok) {
        setSubmitStatus('ok')
        const { getMyRank } = await import('./supabase')
        const r = await getMyRank(score)
        if (r.ok) setGlobalRank(r.rank)
      } else {
      ...
      <motion.div {...pageAnim} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <AnimatePresence>
            {submitStatus === 'loading' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
                  background: 'var(--bg-3)', borderRadius: 20, padding: '5px 14px',
            ...
            {submitStatus === 'ok' && globalRank && (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                transition={{ type: 'spring', stiffness: 400 }}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
                  background: 'rgba(232,1,10,0.1)', border: '1px solid rgba(232,1,10,0.25)',
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Btn variant="secondary" onClick={() => { play('click'); go('leaderboard') }} onHover={() => play('hover')}>
            🏆 Leaderboard
          </Btn>
...
export default function App() {
  ...
  const back = () => {
    const map = { categories: 'welcome', howtoplay: 'welcome', stats: 'welcome',
      leaderboard: 'welcome', game: 'categories', results: 'categories' }
    setScreen(map[screen] || 'welcome')
  }
  const showBack = ['categories','howtoplay','stats','game','results','leaderboard'].includes(screen)
    ...
    <div onClick={doInit} style={{ minHeight: '100vh', maxWidth: 480, margin: '0 auto',
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <AnimatePresence mode="wait">
          ...
          {screen === 'leaderboard' && (
            <motion.div key="leaderboard" {...pageAnim}>
              <div style={{ padding: '72px 16px 30px' }}>
                <LeaderboardScreen
                  myScore={result?.score}
                  myName={result?.mode?.name}
                  play={safePlay}/>
              </div>
            </motion.div>
          )}
